...
#JI     self.latent_size = latent_size = latent_size / 2 #Tree and Mol has two vectors
...
#JI     self.assm_loss = nn.CrossEntropyLoss(size_average=False)
    ...
    def dfs_assemble(self, y_tree_mess, x_mol_vecs, all_nodes, cur_mol, global_amap, fa_amap, cur_node, fa_node, prob_decode, check_aroma):
        ...
        if len(cands) > 1:
            ...
            scores = torch.mv(cand_vecs, x_mol_vecs).cuda() + aroma_score
        else:
        ...
        """
        new from author below
        """
        
        
    def fast_recon_eval(self, smiles, rec):
...
#JI         for i in range(rec):
    ...
    def dfs_assemble_2(self, iter, y_tree_mess, x_mol_vecs, all_nodes, cur_mol, global_amap, fa_amap, cur_node, fa_node, prob_decode, check_aroma):
        ...
        if len(cands) > 1:
            ...
            fatoms,fbonds,agraph,bgraph,scope = jtmpn_holder
            cand_vecs = self.jtmpn(fatoms, fbonds, agraph, bgraph, scope, y_tree_mess[0])
            scores = torch.mv(cand_vecs, x_mol_vecs).cuda() + aroma_score.cuda()
        else:
            scores = torch.Tensor([1.0])
