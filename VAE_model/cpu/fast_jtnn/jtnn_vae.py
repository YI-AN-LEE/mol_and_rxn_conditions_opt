...
class JTNNVAE(nn.Module):
    ...
    def sample_prior(self, prob_decode=False):
        z_tree = torch.randn(1, self.latent_size).cpu()
        z_mol = torch.randn(1, self.latent_size).cpu()
    ...
    def dfs_assemble(self, y_tree_mess, x_mol_vecs, all_nodes, cur_mol, global_amap, fa_amap, cur_node, fa_node, prob_decode, check_aroma):
        ...
        aroma_score = torch.Tensor(aroma_score).cpu()
...
#JI
#JI ******* New codes below ****************
#JI ******* Some of these are experimental *
#JI
    ...
    def fast_recon_eval(self, smiles, rec, radius):
        tree_batch = [MolTree(s) for s in smiles]
        _, jtenc_holder, mpn_holder = tensorize(tree_batch, self.vocab, assm=False)
        tree_vecs, _, mol_vecs = self.encode(jtenc_holder, mpn_holder)
        ...
        for i in range(rec):
            ...
            tree_vec = tree_mean + torch.exp(tree_log_var) * epsilon * radius
            epsilon = create_var(torch.randn(1, self.latent_size), False)
            mol_vec = mol_mean + torch.exp(mol_log_var) * epsilon * radius
            #for i in range(rec):
            new_smiles = self.decode(tree_vec, mol_vec, prob_decode=False)
            smiles_recon.append(new_smiles)

        return smiles_recon
   
    def fast_recon_eval_2(self, smiles, rec):
        tree_batch = [MolTree(s) for s in smiles]
        _, jtenc_holder, mpn_holder = tensorize(tree_batch, self.vocab, assm=False)
        tree_vecs, _, mol_vecs = self.encode(jtenc_holder, mpn_holder)
...
#JI         for i in range(rec):
            new_smiles = self.decode_2(tree_vec, mol_vec, prob_decode=False)
            smiles_recon.append(new_smiles)

        return smiles_recon

    def fast_recon_eval_3(self, smiles, rec):
        tree_batch = [MolTree(s) for s in smiles]
        _, jtenc_holder, mpn_holder = tensorize(tree_batch, self.vocab, assm=False)
        tree_vecs, _, mol_vecs = self.encode(jtenc_holder, mpn_holder)

        tree_vec = self.T_mean(tree_vecs)
#JI     tree_log_var = -torch.abs(self.T_var(tree_vecs)) #Following Mueller et al.
        mol_vec = self.G_mean(mol_vecs)
#JI     mol_log_var = -torch.abs(self.G_var(mol_vecs)) #Following Mueller et al.

        smiles_recon = []
        for i in range(rec):
#JI         epsilon = create_var(torch.randn(1, self.latent_size), False)
#JI         tree_vec = tree_mean + torch.exp(tree_log_var) * epsilon
#JI         epsilon = create_var(torch.randn(1, self.latent_size), False)
#JI         mol_vec = mol_mean + torch.exp(mol_log_var) * epsilon
#JI         for i in range(rec):
            new_smiles = self.decode_2(tree_vec, mol_vec, prob_decode=False)
            smiles_recon.append(new_smiles)
    ...
    def decode_2(self, x_tree_vecs, x_mol_vecs, prob_decode):
        ...
        cur_mol,_, iter = self.dfs_assemble_2(iter, tree_mess, x_mol_vecs, pred_nodes, cur_mol, global_amap, [], pred_root, None, prob_decode, check_aroma=True)

        if cur_mol is None:
            cur_mol = copy_edit_mol(pred_root.mol)
            global_amap = [{}] + [{} for node in pred_nodes]
            global_amap[1] = {atom.GetIdx():atom.GetIdx() for atom in cur_mol.GetAtoms()}
            cur_mol,pre_mol, iter = self.dfs_assemble_2(iter,tree_mess, x_mol_vecs, pred_nodes, cur_mol, global_amap, [], pred_root, None, prob_decode, check_aroma=False)
            if cur_mol is None: cur_mol = pre_mol
    ...
    def dfs_assemble_2(self, iter, y_tree_mess, x_mol_vecs, all_nodes, cur_mol, global_amap, fa_amap, cur_node, fa_node, prob_decode, check_aroma):
        ...
        for i in range(cand_idx.numel()):
            ...
            for nei_node in children:
                if nei_node.is_leaf: continue
                tmp_mol, tmp_mol2, iter = self.dfs_assemble_2(iter, y_tree_mess, x_mol_vecs, all_nodes, cur_mol, new_global_amap, pred_amap, nei_node, cur_node, prob_decode, check_aroma)
                if tmp_mol is None:
                    has_error = True
                    if i == 0: pre_mol = tmp_mol2
                    break
            ...
            if iter > 10000:
               ...
               if not has_error: return cur_mol, cur_mol, iter

        return None, pre_mol, iter
