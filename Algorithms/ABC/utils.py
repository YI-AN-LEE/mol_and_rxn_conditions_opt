...
def adjust_position_and_get_smiles_aryl_freeze(total_paticle_position, bounds, radius, transform:AryTransform, latent_size, JTVAE, freeze):
    random_noise = torch.randn_like(total_paticle_position) * radius #radius is the std of the noise
    # scaling_factors = torch.tensor([random.uniform(0, 1) for _ in range(len(total_paticle_position))]).to('cuda:0')
    scaling_factors = torch.full((total_paticle_position.shape[1],), 2.0, device='cuda:0') # 2 is the std of the vae ls vector
    for i in range(-bounds.shape[0], 0):
    ...
    if freeze == 'proc':
        perturb = torch.zeros_like(total_paticle_position)
        perturb[:, :latent_size] = random_noise[:, :latent_size] * scaling_factors[:, :latent_size]
        total_paticle_position = total_paticle_position + perturb
...
def adjust_position_and_get_smiles_freeze(total_paticle_position, bounds, radius, transform:PvkTransform, latent_size, JTVAE, freeze):
    random_noise = torch.randn_like(total_paticle_position) * radius #radius is the std of the noise
    # scaling_factors = torch.tensor([random.uniform(0, 1) for _ in range(len(total_paticle_position))]).to('cuda:0')
    scaling_factors = torch.full((total_paticle_position.shape[1],), 2.0, device='cuda:0') # 2 is the std of the vae ls vector
    for i in range(-bounds.shape[0], 0):
...
def create_swarm_abc_freeze(center_position, pop_size, bounds, radius, transform:PvkTransform, predictor:Pvk_Ensemble_Predictor, proc_feature_list, latent_size, JTVAE, freeze):
    # Generate the initial swarm.
    total_particle_position = torch.stack([center_position] * pop_size) #大大向量
    print('Finished position generating')
    print('Start generating inital SMILES')
    total_particle_position, total_smiles = adjust_position_and_get_smiles_freeze(total_particle_position, bounds, radius, transform, latent_size, JTVAE, freeze) #add noise and decode
    total_fitness, total_propertys = predictor.ensemble_predict(total_particle_position) #get the vector of all fitness and propertys
    swarm = [Bee(position, bounds) for position in total_particle_position]
...
def create_swarm_abc_aryl_freeze(center_position, pop_size, bounds, radius, transform:AryTransform, predictor:AryEnsmblePredictor, proc_feature_list, latent_size, JTVAE, freeze):
    # Generate the initial swarm.
    total_particle_position = torch.stack([center_position] * pop_size) #大大向量
    print('Finished position generating')
    print('Start generating initial SMILES')
    total_particle_position, total_smiles = adjust_position_and_get_smiles_aryl_freeze(total_particle_position, bounds, radius, transform, latent_size, JTVAE, freeze) #add noise and decode
    total_fitness, total_propertys = predictor.ensemble_predict(total_particle_position) #get the vector of all fitness and propertys
    swarm = [Bee(position, bounds) for position in total_particle_position]
