...
class Particle:
    ...
    def update_velocity_freeze(self, global_best_position, inertia_weight, cognitive_weight, social_weight, freeze = None):
        r1 = random.uniform(0, 1)
        r2 = random.uniform(0, 1)
        if freeze =='mol':
            cognitive_component = cognitive_weight * r1 * (self.best_position - self.position) # personal best
            social_component = social_weight * r2 * (global_best_position - self.position) # global best
            self.velocity = inertia_weight * self.velocity + cognitive_component + social_component
            self.velocity[:32] = 0 #fix molecular part
            self.velocity = self.velocity
        if freeze == 'proc':
            cognitive_component = cognitive_weight * r1 * (self.best_position - self.position) # personal best
            social_component = social_weight * r2 * (global_best_position - self.position) # global best
            self.velocity = inertia_weight * self.velocity + cognitive_component + social_component
            self.velocity[32:] = 0 #fix process part
            self.velocity = self.velocity
        # self.velocity[:32] = 0
