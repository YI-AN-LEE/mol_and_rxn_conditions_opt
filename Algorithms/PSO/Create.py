...
def adjust_position_and_get_smiles_freeze(total_paticle_position, bounds, radius, transform:PvkTransform, freeze):
    random_noise = torch.randn_like(total_paticle_position) * radius
    #scaling_factors = torch.tensor([random.uniform(0, 1) for _ in range(len(total_paticle_position))]).to('cuda:0')
    #total_paticle_position = total_paticle_position + random_noise * scaling_factors.unsqueeze(1)
    # Check the range of all initial process condition.
    ...
    if freeze == 'proc':
        ...
        perturb[:, :32] = random_noise[:, :32] * scaling_factors[:, :32]
...
def create_pool_freeze(center_position, pop_size, bounds, radius, transform:AryTransform, predictor:AryEnsmblePredictor, proc_feature_list, freeze) -> List[Particle]:
    # Generate the initial pool.
    total_particle_position = torch.stack([center_position] * pop_size)
    total_particle_position, total_smiles = adjust_position_and_get_smiles_freeze(total_particle_position, bounds, radius, transform, freeze)
    total_process_position = total_particle_position[:, 32:]
    #feature_df = transform.to_dataframe(total_smiles, total_process_position, proc_feature_list)
    total_fitness, total_propertys = predictor.ensemble_predict(total_particle_position)
    pool = [Particle(position = position, bounds = bounds, smiles = smiles) for position, smiles in zip(total_particle_position, total_smiles)]
